...
class RepoKnowledgeGraph:
    ...
    def _initial_parse_path(self, path: str):
        ...
        for file_path in level1_node_contents:
            ...
            for i, chunk in enumerate(parsed_content):
                ...
                embedding = self.model_service.embed_chunk_code(chunk_node.get_field_to_embed())
    ...
    @classmethod
    def from_dict(cls, data_dict, model_service, index_nodes:bool=True, use_embed:bool=True):
        ...
        if index_nodes:
            self.code_index = CodeIndex(list(self), model_service=model_service, use_embed=use_embed)
