...
class CodeIndex:
    def __init__(self, nodes, model_service, use_embed:bool = True):
    ...
    def index_node(self, node: ChunkNode, use_embed:bool=True) -> ChunkNode:
        ...
        if node.embedding is None or not use_embed:
            print(node.get_field_to_embed())
            embedding = self.model_service.embed_chunk_code(node.get_field_to_embed())
